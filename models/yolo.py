...
class IOLNFADetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nfa_cfg=None, ch=()):  # detection layer
        ...
        self.sigm_alpha = float(nfa_cfg.get('sigm_alpha', 0.5))
        self.nfa_score_bias = float(nfa_cfg.get('score_bias', -2.0))
        self.use_score_adapter = bool(nfa_cfg.get('score_adapter', False))
        self.score_adapter_channels = int(nfa_cfg.get('score_adapter_channels', 64))
        self.score_adapter = None
        if self.use_score_adapter:
            self.score_adapter = nn.ModuleList(
                nn.Sequential(
                    Conv(x, self.score_adapter_channels, 1, 1, act=nn.LeakyReLU(0.1)),
                    Conv(self.score_adapter_channels, self.score_adapter_channels, 3, 1, act=nn.LeakyReLU(0.1)),
                    Conv(self.score_adapter_channels, self.score_adapter_channels, 3, 1, act=nn.LeakyReLU(0.1)),
                    nn.Conv2d(self.score_adapter_channels, self.score_adapter_channels, 1, bias=True),
                )
                for x in ch
            )
            for adapter in self.score_adapter:
                nn.init.constant_(adapter[-1].bias, self.nfa_score_bias)
        self.use_scale_attention = bool(nfa_cfg.get('scale_attention', False))
        self.scale_attention_kernel = max(1, int(nfa_cfg.get('scale_attention_kernel', 3)))
        if self.scale_attention_kernel % 2 == 0:
            self.scale_attention_kernel += 1
        self.scale_attention = None
        if self.use_scale_attention and self.nl > 1:
            self.scale_attention = nn.Conv1d(
                1,
                1,
                kernel_size=self.scale_attention_kernel,
                padding=self.scale_attention_kernel // 2,
                bias=False,
            )
            nn.init.zeros_(self.scale_attention.weight)
    ...
    def _forward_impl(self, x, fused=False):
        ...
        if self.objectness_mode == 'baseline':
            ...
            significance_maps = [
                self._compute_nfa_significance(features[i], pred, i, num_tests)
                for i, pred in enumerate(raw_preds)
            ]
            scale_weights = self._compute_scale_weights(significance_maps)
            preds = [
                self._apply_objectness(raw_preds[i], significance_maps[i], num_tests, scale_weights, i)
                for i in range(self.nl)
            ]
    ...
    def _apply_objectness(self, pred, significance, num_tests, scale_weights, level_idx):
        if self.objectness_mode not in ('olnfa', 'blend'):
            raise ValueError(f'Unsupported objectness_mode={self.objectness_mode}')

        if scale_weights is not None:
            weight = scale_weights[:, level_idx].view(-1, 1, 1, 1)
            significance = significance * weight

        nfa_prob = self._significance_to_objectness(significance, num_tests)
    ...
    def _compute_nfa_significance(self, feature_map, pred, level_idx, num_tests):
        pred_f = pred.float()
        feature_map = self._adapt_feature_map(feature_map, level_idx).float()
        ...
        # NFAU1 treats channels as the third axis of the tested ROI volume.
        box_volume = box_area * float(channels)

        rois = torch.cat((batch_indices[:, None].float(), boxes), dim=1)
        roi_chunk = rois.shape[0] if self.roi_chunk <= 0 else self.roi_chunk
        global_density = feature_map.sigmoid().mean(dim=(1, 2, 3)).clamp(self.eps, 1.0 - self.eps)
        ...
        for start in range(0, rois.shape[0], roi_chunk):
            ...
            roi_sample_count = roi_features.shape[1] * roi_features.shape[-2] * roi_features.shape[-1]
            nu = box_volume[start:stop]
            significance_maps.append(self._compute_significance(kappa, roi_sample_count, nu, p, num_tests))

        bs, na, ny, nx, _ = pred.shape
        return torch.cat(significance_maps, dim=0).view(bs, na, ny, nx)

    def _compute_scale_weights(self, significance_maps):
        if self.scale_attention is None:
            return None
        # Treat the per-scale significance maps as channels and learn one weight per scale.
        descriptors = torch.stack([s.mean(dim=(1, 2, 3)) for s in significance_maps], dim=1)
        attn_dtype = self.scale_attention.weight.dtype
        logits = self.scale_attention(descriptors.unsqueeze(1).to(dtype=attn_dtype)).squeeze(1).float()
        return 2.0 * torch.sigmoid(logits)

    def _adapt_feature_map(self, feature_map, level_idx):
        if self.score_adapter is None:
            return feature_map
        return self.score_adapter[level_idx](feature_map)
    ...
    def _compute_significance(self, kappa, roi_sample_count, nu, p, num_tests):
        ...
        return torch.where(q > p, nu * kl - log_eta, -log_eta.expand_as(q))

    def _significance_to_objectness(self, significance, num_tests):
        log_eta = significance.new_tensor(float(num_tests)).log()
        return (2.0 * torch.sigmoid(self.sigm_alpha * (significance + log_eta)) - 1.0).clamp(0.0, 1.0)
